...
"""
    LLVMGetOrInsertFunction(M, Name, NameLen, FunctionTy)
...
"""
function LLVMGetOrInsertFunction(M, Name, NameLen, FunctionTy)
    ccall((:LLVMGetOrInsertFunction, libllvm), LLVMValueRef, (LLVMModuleRef, Cstring, Csize_t, LLVMTypeRef), M, Name, NameLen, FunctionTy)
end
...
function LLVMConstFPFromBits(Ty, N)
    ccall((:LLVMConstFPFromBits, libllvm), LLVMValueRef, (LLVMTypeRef, Ptr{UInt64}), Ty, N)
end
...
function LLVMConstDataArray(ElementTy, Data, SizeInBytes)
    ccall((:LLVMConstDataArray, libllvm), LLVMValueRef, (LLVMTypeRef, Ptr{Cvoid}, Csize_t), ElementTy, Data, SizeInBytes)
end
...
"""
    LLVMGlobalAddMetadata(Global, Kind, MD)
...
"""
function LLVMGlobalAddMetadata(Global, Kind, MD)
    ccall((:LLVMGlobalAddMetadata, libllvm), Cvoid, (LLVMValueRef, Cuint, LLVMMetadataRef), Global, Kind, MD)
end
...
"""
function LLVMGlobalAddDebugInfo(Global, GVE)
    ccall((:LLVMGlobalAddDebugInfo, libllvm), Cvoid, (LLVMValueRef, LLVMMetadataRef), Global, GVE)
end
...
function LLVMDbgRecordGetDebugLoc(Rec)
    ccall((:LLVMDbgRecordGetDebugLoc, libllvm), LLVMMetadataRef, (LLVMDbgRecordRef,), Rec)
end

function LLVMDbgRecordGetKind(Rec)
    ccall((:LLVMDbgRecordGetKind, libllvm), LLVMDbgRecordKind, (LLVMDbgRecordRef,), Rec)
end

"""
    LLVMDbgVariableRecordGetValue(Rec, OpIdx)
...
function LLVMDbgVariableRecordGetValue(Rec, OpIdx)
    ccall((:LLVMDbgVariableRecordGetValue, libllvm), LLVMValueRef, (LLVMDbgRecordRef, Cuint), Rec, OpIdx)
end

"""
    LLVMDbgVariableRecordGetVariable(Rec)
...
function LLVMDbgVariableRecordGetVariable(Rec)
    ccall((:LLVMDbgVariableRecordGetVariable, libllvm), LLVMMetadataRef, (LLVMDbgRecordRef,), Rec)
end

"""
    LLVMDbgVariableRecordGetExpression(Rec)
...
function LLVMDbgVariableRecordGetExpression(Rec)
    ccall((:LLVMDbgVariableRecordGetExpression, libllvm), LLVMMetadataRef, (LLVMDbgRecordRef,), Rec)
end
...
"""
    LLVMGetSwitchCaseValue(SwitchInstr, i)
...
"""
function LLVMGetSwitchCaseValue(SwitchInstr, i)
    ccall((:LLVMGetSwitchCaseValue, libllvm), LLVMValueRef, (LLVMValueRef, Cuint), SwitchInstr, i)
end

"""
    LLVMSetSwitchCaseValue(SwitchInstr, i, CaseValue)
...
"""
function LLVMSetSwitchCaseValue(SwitchInstr, i, CaseValue)
    ccall((:LLVMSetSwitchCaseValue, libllvm), Cvoid, (LLVMValueRef, Cuint, LLVMValueRef), SwitchInstr, i, CaseValue)
end
...
function LLVMGetVolatile(Inst)
    ccall((:LLVMGetVolatile, libllvm), LLVMBool, (LLVMValueRef,), Inst)
end
...
"""
@cenum LLVMChecksumKind::UInt32 begin
    CSK_MD5 = 0
    CSK_SHA1 = 1
    CSK_SHA256 = 2
end
...
"""
    LLVMDIBuilderCreateFileWithChecksum(Builder, Filename, FilenameLen, Directory, DirectoryLen, ChecksumKind, Checksum, ChecksumLen, Source, SourceLen)
...
"""
function LLVMDIBuilderCreateFileWithChecksum(Builder, Filename, FilenameLen, Directory, DirectoryLen, ChecksumKind, Checksum, ChecksumLen, Source, SourceLen)
    ccall((:LLVMDIBuilderCreateFileWithChecksum, libllvm), LLVMMetadataRef, (LLVMDIBuilderRef, Cstring, Csize_t, Cstring, Csize_t, LLVMChecksumKind, Cstring, Csize_t, Cstring, Csize_t), Builder, Filename, FilenameLen, Directory, DirectoryLen, ChecksumKind, Checksum, ChecksumLen, Source, SourceLen)
end
...
"""
    LLVMParseIRInContext2(ContextRef, MemBuf, OutM, OutMessage)

Read LLVM IR from a memory buffer and convert it into an in-memory Module object. Returns 0 on success. Optionally returns a human-readable description of any errors that occurred during parsing IR. OutMessage must be disposed with [`LLVMDisposeMessage`](@ref). The memory buffer is not consumed by this function. It is the responsibility of the caller to free it with [`LLVMDisposeMemoryBuffer`](@ref).

# See also
llvm::ParseIR()
"""
function LLVMParseIRInContext2(ContextRef, MemBuf, OutM, OutMessage)
    ccall((:LLVMParseIRInContext2, libllvm), LLVMBool, (LLVMContextRef, LLVMMemoryBufferRef, Ptr{LLVMModuleRef}, Ptr{Cstring}), ContextRef, MemBuf, OutM, OutMessage)
end
...
Install the plugin that submits debug objects to the executor. Executors must expose the llvm\\_orc\\_registerJITLoaderGDBAllocAction symbol.
"""
...
The memory buffer is not consumed by this function. It is the responsibility of the caller to free it with [`LLVMDisposeMemoryBuffer`](@ref).
...
The binary file does not own its backing buffer. It is the responsibility of the caller to free it with [`LLVMDisposeMemoryBuffer`](@ref).
"""
...
"""
    LLVMOrcCreateObjectLinkingLayerWithInProcessMemoryManager(Result, ES)

Create a ObjectLinkingLayer instance using the standard JITLink InProcessMemoryManager for memory management.
"""
function LLVMOrcCreateObjectLinkingLayerWithInProcessMemoryManager(Result, ES)
    ccall((:LLVMOrcCreateObjectLinkingLayerWithInProcessMemoryManager, libllvm), LLVMErrorRef, (Ptr{LLVMOrcObjectLayerRef}, LLVMOrcExecutionSessionRef), Result, ES)
end
...
"""
    LLVMOrcCreateRTDyldObjectLinkingLayerWithSectionMemoryManagerReserveAlloc(ES, ReserveAlloc)
...
"""
function LLVMOrcCreateRTDyldObjectLinkingLayerWithSectionMemoryManagerReserveAlloc(ES, ReserveAlloc)
    ccall((:LLVMOrcCreateRTDyldObjectLinkingLayerWithSectionMemoryManagerReserveAlloc, libllvm), LLVMOrcObjectLayerRef, (LLVMOrcExecutionSessionRef, LLVMBool), ES, ReserveAlloc)
end
